...
public class RegAPI2 : MonoBehaviour
{
    ...
    void HandleRegistrationResponse(string responseText)
    {
        ...
        try
        {
            ...
            if (responseJson.TryGetValue("OTP-status", out JToken otpStatusToken) && otpStatusToken.Type == JTokenType.Integer)
            {
                int otpStatus = otpStatusToken.Value<int>();

                if (otpStatus == 200)
                {
                    string message = responseJson.TryGetValue("message", out JToken messageToken) ? messageToken.Value<string>() : "";
                    userID = responseJson.TryGetValue("user_id", out JToken userIdToken) ? userIdToken.Value<int>() : 0;
                ...
                {
                    Debug.LogError("Registration failed. OTP status: " + otpStatus);
                    Debug.LogError(Convert.ToBoolean("Message: " + responseJson.TryGetValue("message", out JToken errorMessageToken)) ? errorMessageToken.Value<string>() : "");
                }
            }
            else
            {
                Debug.LogError("Invalid OTP status in JSON response.");
            }
        }
        catch (Exception ex)
        {
